    ...
    const enriched = await Promise.all(
      customers.map(async (c) => {
        ...
        if (Bill) {
          ...
          obj.totalSpend   = bills.reduce((s, b) => s + (b.txnType === 'give' ? -b.amount : (b.txnType === 'receive' && b.status === 'paid' ? b.amount : 0)), 0);
...
router.get('/:id/timeline', roleGuard(['admin', 'editor', 'support']), async (req, res, next) => {
  try {
    ...
    const totalSpend = bills.reduce((s, b) => s + (b.txnType === 'give' ? -b.amount : (b.txnType === 'receive' && b.status === 'paid' ? b.amount : 0)), 0);
